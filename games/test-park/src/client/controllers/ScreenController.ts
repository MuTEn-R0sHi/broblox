/**
 * Screen Controller — Test Park (Client)
 *
...
import type { ShopProduct, ShopGamePass } from "@broblox/ui";
...
/** Shop product/pass definitions (mirrors MarketplaceService config) */
...
const SHOP_PASSES: ShopGamePass[] = [
...
interface ScreenHandles {
  ...
  shop?: ReturnType<typeof createShopScreen>;
}
...
export const ScreenController: Controller & {
  ...
  onStart() {
    ...
    // Shop Screen
    screens.shop = createShopScreen(screenGui, {
      getProducts: () => SHOP_PRODUCTS,
      getPasses: () => SHOP_PASSES,
      getBalance: () => cachedData?.coins ?? 0,
      checkPassOwnership: (passId) => {
        return RemoteController.checkGamePass(passId);
      },
      onBuyProduct: (productId) => {
        RemoteController.buyProduct(productId);
        task.spawn(() => refreshData());
      },
      onBuyPass: (passId) => {
        RemoteController.buyProduct(passId);
        task.spawn(() => refreshData());
      },
      onOpen: () => {
        // Tutorial hook: fire "open_shop" action for ftue_shop sequence
        logger.debug("Shop opened — firing tutorial action open_shop");
      },
      onPurchase: () => {
        // Tutorial hook: fire "first_purchase" action for ftue_shop sequence
        logger.debug("Shop purchase — firing tutorial action first_purchase");
      },
      onClose: () => hideActiveModal(),
    });
