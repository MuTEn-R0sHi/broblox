/**
 * TelemetryService Tests — Obby Game
 *
 ...
 * - Exposes trackPurchase, trackStageComplete, trackPlayerDeath helpers
 */

import { describe, it, expect, vi, beforeEach } from "vitest";

describe("TelemetryService (obby)", () => {
  ...
  it("trackPurchase emits economy event and increments counter", async () => {
    ...
    mod.trackPurchase(player, "100 Coins", 3_000_001, 25);
  ...
  it("trackStageComplete emits event and increments counter", async () => {
    const mod = await loadAndStart();
    const player = { UserId: 42, Name: "TestPlayer" } as unknown as Player;
    mod.trackStageComplete(player, 5, 12.5, true);
  ...
  it("trackPlayerDeath emits event and increments counter", async () => {
    const mod = await loadAndStart();
    const player = { UserId: 42, Name: "TestPlayer" } as unknown as Player;
    mod.trackPlayerDeath(player, 3, "fall");
